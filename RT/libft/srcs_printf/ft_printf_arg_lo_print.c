...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/13 18:15:53 by dcooray           #+#    #+#             */
/*   Updated: 2017/03/13 18:15:54 by dcooray          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_printf_arg_lo_print(int *i, t_var *var)
{
	...
	if (var->flag_hash && !var->llo)
	{
		*i += 1;
		ft_putchar('0');
	}
	else if (var->flag_precision && !var->precision && !var->llo)
		ft_printf_arg_olo_implicit_precision(var, i, &j);
	else if (!var->champ && var->precision)
		ft_printf_arg_lo_print_ter(i, var);
	else if (var->champ)
		ft_printf_arg_lo_print_bis(i, var);
	else
	{
		if (var->flag_hash)
		{
...
void	ft_printf_arg_lo_print_bis(int *i, t_var *var)
{
	...
	if (j > 0)
	{
		...
		*i += j;
		...
		ft_printf_arg_lo_print_quater(i, j, var);
	}
	else
		ft_printf_arg_lo_print_ter(i, var);
}
	...
	if (var->precision)
	{
		...
		if (j > 0)
		{
			...
			*i += ft_strlen(var->s);
			...
			ft_printf_arg_lo_print_quinque(i, var);
	}
	else
		ft_printf_arg_lo_print_quinque(i, var);
}
